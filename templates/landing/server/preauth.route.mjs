// LaunchThesis validate — reference preauth route (Stripe manual-capture, Vercel/Node).
...
// STRIPE_SECRET_KEY (LIVE), LT_PRICE_CENTS, LT_CURRENCY (default usd).
export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).json({ error: "POST only" });
  const amountCents = Number(process.env.LT_PRICE_CENTS);
  if (!Number.isFinite(amountCents) || amountCents <= 0) return res.status(500).json({ error: "LT_PRICE_CENTS not set" });
  const params = new URLSearchParams({
    amount: String(amountCents),
    currency: process.env.LT_CURRENCY || "usd",
