// LaunchThesis validate — client capture. Cookieless (a per-tab sessionStorage id only,
...
function sessionId() {
  let s = sessionStorage.getItem("lt_sid");
  if (!s) { s = "s_" + Math.random().toString(36).slice(2) + Date.now().toString(36); sessionStorage.setItem("lt_sid", s); }
  return s;
}
...
export async function capture(tier, { email = null, amount = 0, live = false } = {}) {
  ...
  try {
    ...
    console.error("[launchthesis] capture failed:", e);
